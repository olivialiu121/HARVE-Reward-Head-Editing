...
This sweep is an INPUT to α* selection, so it must run before
src.harve.run_harve.select_alpha_star (the pipeline in src.harve.run_harve
invokes it for you). After the sweep it reports, for information only, whether
the operating point currently on record satisfies the guard (drop ≤
|constraint_delta| pp).
"""
...
def run_rewardbench(rm_key: str, alphas: list[float], *, output_dir: Path,
    ...
    Idempotent: an existing sweep is reused unless ``force``. This is what makes
    it safe for src.harve.run_harve to call as its guard step.
    """
    ...
    if out_path.exists() and not force:
        ...
        have = {round(float(s["alpha"]), 6) for s in cached["sweep"]}
        if all(round(float(a), 6) in have for a in alphas):
            print(f"[rewardbench] {out_path.name} covers every requested α — skipping")
            return cached
        print(f"[rewardbench] {out_path.name} is missing some α — recomputing")
    ...
    # --- guard check: does the operating point keep the drop within budget? ---
    # Informational only. This sweep is an INPUT to selection (see
    # src.harve.run_harve.select_alpha_star); the α reported here is whatever
    # operating point is currently on record.
    base = next((s["micro"] for s in sweep if abs(s["alpha"]) < 1e-9), None)
    sel_path = REPO_ROOT / "runs" / "harve" / f"{rm_key}_alpha_star.json"
    if sel_path.exists():
        alpha_star = float(json.loads(sel_path.read_text())["alpha_star"])
        src = "selected on dev"
    else:
        alpha_star = spec.get("alpha_star")
        src = "configs/rms.yaml"
    delta = harve_cfg["alpha_star_selection"]["constraint_delta"]  # e.g. -4.0
    at_star = (None if (alpha_star is None or base is None) else
               next((s["micro"] for s in sweep
                     if abs(s["alpha"] - alpha_star) < 1e-9), None))
    if at_star is not None:
        drop = base - at_star
        ok = drop <= -delta + 1e-9
        print(f"[rewardbench] α*={alpha_star} ({src}): general drop = {drop:+.2f}pp "
