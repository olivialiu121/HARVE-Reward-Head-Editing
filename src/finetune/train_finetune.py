...
def main():
    ...
    random.Random(ft_cfg["seed"] + 1).shuffle(train_data)
    ...
    # Derived fields: dev-loss best-model selection needs matching step
    # strategies, and the Trainer re-seeds from args.seed at train start.
    rc_kwargs.update(seed=ft_cfg["seed"],
                     eval_strategy="steps", save_strategy="steps")
    train_args = RewardConfig(output_dir=args.output, **rc_kwargs)
    # TRL renamed RewardTrainer's `tokenizer` argument to `processing_class`.
    # Pick whichever this install actually accepts.
    rt_params = inspect.signature(RewardTrainer.__init__).parameters
    tok_kwarg = "processing_class" if "processing_class" in rt_params else "tokenizer"
