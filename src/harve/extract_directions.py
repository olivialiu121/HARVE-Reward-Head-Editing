...
The fooled flag is the reward model's OWN score for the pair (the head's
forward output, in the model's native dtype) -- not a float32 re-derivation of
h . w_r. The two disagree on a handful of near-tied pairs, and since a
subcategory with fewer than ``min_fooled_per_subcat`` fooled pairs is dropped
from the subspace entirely, a single flip can change which directions exist.
...
@torch.no_grad()
def _hidden_and_score(model, tok, question: str, response: str,
                      *, max_length: int | None = None) -> tuple[torch.Tensor, float]:
    """Forward one (question, response); return the final-layer last-token
    hidden state (float32, CPU) and the reward model's own scalar score."""
    ...
    h = out.hidden_states[-1][0, pos, :].float().cpu()
    logits = out.logits if hasattr(out, "logits") else out
    return h, float(logits.squeeze().float().item())
...
def extract_direction(model, tok, w_r: torch.Tensor, pairs: list[dict],
                      *, restrict_to_fooled: bool = True,
                      min_fooled: int = 3,
                      unit_norm: bool = True,
                      max_length: int | None = None) -> tuple[torch.Tensor | None, dict]:
    ...
    for r in tqdm(pairs, desc="    extracting", leave=False):
        h_g, s_g = _hidden_and_score(model, tok, r["question"], r["gold_response"],
                                     max_length=max_length)
        h_h, s_h = _hidden_and_score(model, tok, r["question"], r["hacked_response"],
                                     max_length=max_length)
        deltas.append(h_h - h_g)
...
def extract_all_directions(rm_key: str, rms_cfg: dict, harve_cfg: dict,
    ...
    # One load: the full RM gives us the hidden states, the RM's own scores
    # (for the fooled flag) and the score-head weight w_r that HARVE edits.
    print(f"  Loading model {spec['hf_id']} (body + score head)…")
    model, tok, w_r = load_rm_for_scoring(
        spec["hf_id"],
    ...
    edited = [c for c in target_cats if c in v_dict]
    dropped = [c for c in target_cats if c not in v_dict]
    print(f"  Edited subspace: {len(edited)} direction(s) {edited}")
    if dropped:
        print(f"  Dropped (too few fooled train pairs): {dropped}")
